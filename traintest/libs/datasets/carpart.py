...
from datasets.train_test_split import *
...
class CarPart(data.Dataset):
    def __init__(
        ...
        if self.boundary : self.LINE_SIZE=30
        ...
        for i in range(len(self.images)):
            ...
            if self.boundary: cls, label, boundary =  self.parseLabel(img_id, 0, 0)
            else: cls, label =  self.parseLabel(img_id, 0, 0)
            if len(cls)>0:
        ...
        #sampled = random.sample(self.valid_images,98)
        #print(sampled,len(sampled))

        self.phase=phase
        if phase=='train':
            train_temp = list()
            for i in self.valid_images:
                if(i in test_list):
                    continue
                else:
                    train_temp.append(i)
            self.valid_images = train_temp
        elif phase=='test':
            test_temp = list()
            for i in self.valid_images:
                if(i in test_list):
                    test_temp.append(i)
            self.valid_images = test_temp
    ...
    def parseLabel(self, id, max_size, pad):
        ...
        if self.boundary : boundary = np.zeros((max_size, max_size))
            ...
            try:
                ...
                for shape in raw['regions']:
                    ...
                    if 'ellipse' in shape_name:
                        ...
                        if self.boundary:
                            first, last = pts[0][0], pts[0][-1]
                            cv2.line(boundary, (int(first[0]), int(first[1])), (int(last[0]), int(last[1])), (1, 1, 1),  self.LINE_SIZE//2)
                            for j in range(pts[0].shape[0]):
                                if(j<pts[0].shape[0]-1):
                                    now, next = pts[0][j],pts[0][j+1]
                                    cv2.line(boundary, (int(now[0]), int(now[1])), (int(next[0]), int(next[1])), (1, 1, 1),  self.LINE_SIZE//2)
                        points.append(pts)
                        #if part == "Emblem":
    ...
    def __getitem__(self, index):
        ...
        #print("========")
        if self.boundary:
            cls, mask, boundary = self.parseLabel(img_id, max_size, pad)
        else:
        ...
        if self.transform is not None:
            if self.boundary:
                transformed = self.transform(image = img, masks=[mask,boundary])
...
if __name__=="__main__":
    ...
    dataset = CarPart(transform=aug, boundary=True)
    print(len(dataset))
    img, mask, boundary = dataset[55]
